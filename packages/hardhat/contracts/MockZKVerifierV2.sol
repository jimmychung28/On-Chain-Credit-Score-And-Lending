...
contract MockZKVerifierV2 {
     ...
     */
    function verifyProof(bytes calldata proof, uint256[4] calldata publicSignals) external returns (bool) {
    ...
    function _simulateVerification(
        ...
        bytes32 signalsHash = keccak256(
            abi.encodePacked(publicSignals[0], publicSignals[1], publicSignals[2], publicSignals[3])
        );
    ...
    function getVerificationInfo()
        external
        pure
        returns (string memory algorithm, string memory curve, uint256 proofSize, uint256 signalCount)
    {
        return (
            "Mock Groth16",
            "BN254",
